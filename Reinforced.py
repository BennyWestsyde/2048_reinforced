...
def teach_agent(agent, top_episodes):
    ...
    for ep in top_episodes:
        for i, game_state in enumerate(ep.episode_buffer.states):
            next_state = ep.episode_buffer.states[i + 1].state if i + 1 < len(ep.episode_buffer.states) else game_state.state
...
for phase in range(num_phases):
    ...
    epoch_training_details = EpochTrainingDetails(
        ...
        epoch_training_high_tile=max([max([state.state['max_value'] for state in ed.episode_buffer.states]) for ed in training_episodes]),
    ...
    epoch_testing_details = EpochTestingDetails(
        ...
        epoch_testing_high_tile=max([max([state.state['max_value'] for state in ed.episode_buffer.states]) for ed in testing_episodes]),
    ...
    phase_details.phase_high_tile = max([max([state.state['max_value'] for state in ed.episode_buffer.states]) for ed in testing_episodes])
    ...
    top_episodes = sorted(testing_episodes, key=lambda x: x.episode_score, reverse=True)[:int(len(testing_episodes) * (percentage_kept / 100))]
